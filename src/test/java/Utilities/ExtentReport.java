...
public class ExtentReport implements ITestListener {
   ...
   public void onStart(ITestContext context ) {
       ...
       sparkreporter= new ExtentSparkReporter(System.getProperty("user.dir")+"\\Reports\\"+repname);
        sparkreporter.config().setDocumentTitle("CI_TESTING_REPORT");
        sparkreporter.config().setReportName("CI SANITY REPORT");
   ...
   public void onTestFailure(ITestResult result )
   {
	   ...
	   test.log(Status.INFO, "Test  case fail reason is "+result.getThrowable().getMessage());
	     ...
	     // Get WebDriver instance from the result
	      WebDriver driver = ((BaseClass) result.getInstance()).driver;
	   try
	   {
		   String imgpath=new BaseClass().capturescreenshot( driver,result.getName());
